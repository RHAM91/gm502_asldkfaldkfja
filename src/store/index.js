    ...
    getters:{
        g_filtro_alumnos(state){
            if (state.filtros.filtro_almunos.length > 2) {
                ...
                var filtro = state.alumnos.filter(alumno => filtrar_acentos(alumno.nombre.toLowerCase()).match(regex) || filtrar_acentos(alumno.codigo.toLowerCase()).match(regex) || filtrar_acentos(alumno.iglesia.toLowerCase()).match(regex) || alumno.telefono == parametro || filtrar_acentos(alumno.pastor.toLowerCase()).match(regex))
